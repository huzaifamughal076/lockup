...
class _EditActivityScreenState extends State<EditActivityScreen> {
  ...
  Future getMemberDetails() async {
    final databaseReferance = await FirebaseDatabase.instance.reference().child("Users");
    members.clear();
    for (int i = 0; i < memberIds.length; i++) {
      String name = "";
      String email = "";
      String phone = "";
      await databaseReferance.child(memberIds[i]).once().then((value)
      {
        name = value.snapshot.child("name").value.toString();
        name = value.snapshot.child("email").value.toString();
        name = value.snapshot.child("phone").value.toString();
      });
      UserModel user = UserModel(name: name, id: memberIds[i], email: email, phone: phone);
      members.add(user);
    }
  }
